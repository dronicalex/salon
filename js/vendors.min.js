...
// Cелект для блока с табами
// Родителю табов добавить класс .tabs-select
// Тайтлам табов добавить класс .tabs-select__item
// Блокам табов добавить класс .tabs-select-block
setTimeout(() => {
	if (selectOptions = document.querySelector('.tabs-select')) {
		let selectOptions = document.querySelector('.tabs-select').querySelector('.select__options');

		if (selectOptions != null) {
			selectOptions.addEventListener('click', function (e) {
				let tabs = document.querySelectorAll('.tabs-select-block');

				for (let index = 0; index < tabs.length; index++) {
					const tab = tabs[index];
					tab.classList.remove('_active');
				}
				tabs[e.target.dataset.value].classList.add('_active')
			})
		}
	}
...
let tabItems = document.querySelectorAll('.tabs-select__item');
let tabSelect = document.querySelector('.tabs-choice__select');

if (tabItems != null) {
	for (let index = 0; index < tabItems.length; index++) {
		const curent = tabItems[index];

		tabSelect.insertAdjacentHTML('beforeend', `<option value="${index}">${curent.textContent}</option>`)
	}
}

let popularBlock = document.querySelectorAll('.popular__block');
for (let index = 0; index < popularBlock.length; index++) {
	const element = popularBlock[index];
...
let razdelBlock = document.querySelector('.razdel__block');
if (razdelBlock) {
...
let bpo = document.querySelectorAll('._op');

for (let index = 0; index < bpo.length; index++) {
	const element = bpo[index];
